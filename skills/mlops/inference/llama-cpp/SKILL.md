...
metadata:
  hermes:
    tags: [llama.cpp, GGUF, Quantization, Hugging Face Hub, CPU Inference, Apple Silicon, Edge Deployment, AMD GPUs, Intel GPUs, NVIDIA, URL-first, cloud-gpu, modal]
---
...
- Deploy GGUF models on cloud GPU (Modal, RunPod, etc.)
...
## Remote / Cloud GPU Deployment

Deploy llama-cpp-python on cloud GPU services for inference without local hardware.

### When to use

- No local GPU or insufficient VRAM
- Need more throughput than local CPU/GPU can provide
- Want a persistent API endpoint without managing infra
- Running larger models (12B+) that don't fit local hardware

### Supported platforms

| Platform | Free tier | Typical GPU | Cost/hr (T4) | Notes |
|----------|-----------|-------------|-------------|-------|
| **Modal** | $30/mo credits | T4, A100, H100 | ~$0.70 (T4) | Best blend of free credits + CLI + persistent APIs |
| RunPod | None | RTX 4090, A100 | ~$0.50 (RTX 4090) | Raw server, more setup |
| Hugging Face Spaces | CPU 24/7, occasional T4 | T4 | Free (limited) | Gradio/Streamlit only |
| Google Colab | T4 ~12h sessions | T4 | Free | Notebook-based, not persistent |

### Modal quickstart

```bash
pip install modal
modal setup                          # OAuth — opens browser
```

Write a script (e.g. `modal_gemma.py`):
...
app = modal.App("gguf-inference")

@app.function(gpu="t4", image=image, volumes={"/models": volume}, timeout=300)
...
Run:
```bash
modal run modal_gemma.py
```

Deploy as API:
```bash
modal deploy modal_gemma.py
```

### Free tier capacity (Modal, $30/mo)

| GPU | Cost/hr | Approx free hours |
|-----|---------|-------------------|
| T4 | ~$0.70 | ~42h |
| L40S | ~$1.00 | ~30h |
| A100-40GB | ~$2.50 | ~12h |
...
1. Download GGUF file (e.g. via HF Hub or direct URL)
2. Upload to a Modal Volume (`modal volume put`)
3. Define a Modal Image with llama-cpp-python installed
4. Mount the volume, load the model, run inference

### Pitfalls

- **Python 3.14 + Modal:** Modal's dependency chain (grpclib, watchfiles, yarl, aiohttp) can have broken `__init__.py` files on Python 3.14.4. Use Python 3.13 instead: `uv venv --python 3.13 .venv-modal && source .venv-modal/bin/activate && pip install modal`. See the [cloud GPU reference](references/cloud-gpu.md) for details.
- **First cold start:** Modal builds the container image on first run, which can take 2-5 minutes. Subsequent runs use the cached image.
- **Model upload:** Large GGUFs (7GB+) take time to upload to Modal Volume. Use `modal volume put models local_file.gguf /models/model.gguf`.
...
- **[cloud-gpu.md](references/cloud-gpu.md)** — Modal setup, auth, Python 3.14/PRoot workaround, Gemma 4 12B GGUF example, volume management

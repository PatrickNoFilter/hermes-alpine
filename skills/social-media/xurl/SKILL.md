...
> **Common pitfall: credential mismatch.** Users often confuse **OAuth 1.0a credentials** (Consumer Key / Consumer Secret) with **OAuth 2.0 PKCE credentials** (Client ID / Client Secret). They are NOT the same. The X Developer Portal displays them in separate sections. xurl uses OAuth 2.0 PKCE and requires a Client ID + Client Secret. If you register an app using Consumer Key (OAuth 1.0a) in place of Client ID, `xurl auth oauth2` may not work. Confirm you're copying from the **OAuth 2.0 Client ID and Client Secret** section in the portal.

> **Callback URL format:** X's developer portal may reject `http://localhost:8080/callback` as "not a valid callback URL format" in the User Authentication Settings. Try these alternatives in order:
> 1. `http://localhost:8080` (omit `/callback` path)
> 2. `http://localhost:8080/callback/` (trailing slash)
> 3. `http://127.0.0.1:8080/callback` (IP instead of hostname)
> 4. `https://localhost/callback` — X portal now requires **HTTPS** for callback URLs in some regions/app tiers. Even for localhost, `https://localhost/callback` is accepted when `http://` variants are rejected. Update via portal → app → User Authentication Settings → Callback URI, then sync locally:
>    ```bash
>    xurl auth apps redirect-uri set --app my-app https://localhost/callback
>    ```
> If all fail, skip OAuth 2.0 entirely and use **OAuth 1.0a** (Workaround A below) — it needs no callback URL.
>
> **Website URL** is a separate mandatory field in the X Developer Portal (left sidebar → Settings → App info). This is required even for local dev. Use a GitHub profile or any personal URL. Example: `https://github.com/YourUsername`.
>
> **Headless / no-browser environments:** `xurl auth oauth2` starts a local HTTP server on `localhost:8080` and opens a browser for the X authorization page. On a server, SSH session, or Termux without X11/display, there is no browser. Three workarounds, from most capable to least:
>
> **Workaround A — OAuth 1.0a (no browser, full access):** Generate an Access Token + Access Token Secret from the X Developer Portal (app → "Keys and tokens" → "Access Token and Secret"), then configure:
> ```bash
> xurl auth oauth1 --consumer-key YOUR_CONSUMER_KEY --consumer-secret YOUR_CONSUMER_SECRET --access-token YOUR_ACCESS_TOKEN --token-secret YOUR_TOKEN_SECRET
> ```
> OAuth 1.0a gives full read/write access and requires no browser redirect. Tokens are permanent unless revoked. The user generates these from the portal; the agent must NOT handle them inline.
>
> **Workaround B — Copy `~/.xurl` from a machine with a browser:** Run the full OAuth 2.0 PKCE setup on a laptop/desktop, then copy `~/.xurl` to the headless machine.
>
> **Workaround C — Bearer token (no browser, read-only):** App-only auth for public data (user lookup, search, timeline). No posting, liking, or DMing.
> ```bash
> xurl auth app --bearer-token "AAAA...token"
> ```
> Beware: Bearer Tokens from the developer portal often contain URL-encoded characters (`%2B` → `+`, `%3D` → `=`). Shell expansion in `curl` commands can corrupt these — use Python `urllib.request` or write the token to a temp file instead. `xurl auth app` handles the token directly without shell interpolation, so it's safe.
...
| Update app credentials | `xurl auth apps update APP_NAME --client-id ID --client-secret SECRET` |
| Set redirect URI | `xurl auth apps redirect-uri set --app APP_NAME URI` |
...
| Callback URL `http://localhost:8080/callback` rejected as "not a valid callback url format" | X portal UI rejects the format | Try `http://localhost:8080`, `http://localhost:8080/callback/`, or `http://127.0.0.1:8080/callback`. Or switch to OAuth 1.0a (no callback needed). |
| X login page shows "We've temporarily limited your login. Please try again later." | Too many failed password attempts trigger X anti-bot rate limiting | Wait several hours to 24h before retrying. Try correct password from a different device/IP to reset the rate limit. Avoid submitting multiple password variants in rapid succession. |
| `401` on some user lookups but `CreditsDepleted` or `200` on others, same Bearer Token | Account-specific visibility restriction. The 401-returning account may be private, suspended, deactivated, or require user-context auth. The token itself is valid — the endpoint restricts visibility per-account. | Switch to OAuth 1.0a or OAuth 2.0 user-context auth. App-only (Bearer) auth cannot resolve accounts with visibility restrictions, even for basic lookups. Verify which accounts the token *can* reach to distinguish a credential problem from an account problem. |
...
| Consumer Key / Secret not working with OAuth 2.0 | User copied OAuth 1.0a credentials (Consumer Key/Secret) instead of OAuth 2.0 PKCE Client ID/Secret | Go to your app → "Keys and tokens" → scroll to "OAuth 2.0 Client ID and Client Secret" section. The Consumer Key (OAuth 1.0a) is a different credential set and won't work with xurl's default OAuth 2.0 flow. |

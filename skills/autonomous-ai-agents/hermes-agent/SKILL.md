...
```shell
hermes dashboard              # Start on port 9119 (localhost only)
hermes dashboard --port 8080  # Custom port
hermes dashboard --tui        # With embedded chat terminal
...
**Built-in dashboard** (ships with Hermes Agent):
- Location: `/usr/local/lib/hermes-agent/web/`
- Tech: Node.js + TypeScript + Vite (React)
- Start: `hermes dashboard`
- Features: Config management, API key setup, session browsing

**Separate project** ([nesquena/hermes-webui](https://github.com/nesquena/hermes-webui)):
- Tech: Python + vanilla JS (no build step)
- Start: `python server.py` or `docker compose up -d`
- Features: Full CLI parity, workspace browser, three-panel layout
...
| GitHub Copilot ACP | External | `COPILOT_CLI_PATH` or Copilot CLI |\n\n> **Note**: To add custom providers or modify config values, use `hermes config set` (e.g., `hermes config set providers.myprovider.base_url http://...`) rather than editing `config.yaml` directly, as direct file edits are blocked for security reasons.\n\nFull provider docs: https://hermes-agent.nousresearch.com/docs/integrations/providers
...
| Custom/Local Providers | See `references/integrating-custom-providers.md` in this skill |
